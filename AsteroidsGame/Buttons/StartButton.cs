...
{
    public class StartButton : AbstractButtonClass
    {

        public new event onClickHandler onClick;
        public delegate void onClickHandler(EventArgs e);

        public StartButton(Texture2D startButtonSprite, int screenWidth, GraphicsDevice graphicsDevice)
        {
            buttonSprite = startButtonSprite;
            buttonScale = new Vector2(0.8f, 0.8f);
            buttonPosition = new Vector2((screenWidth / 2) - (buttonSprite.Width * buttonScale.X / 2), 280);
