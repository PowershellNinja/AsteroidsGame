...
{
    public class ExitButton : AbstractButtonClass
    {

        public new event onClickHandler onClick;
        public delegate void onClickHandler(EventArgs e);

        public ExitButton(Texture2D exitButtonSprite, int screenWidth, GraphicsDevice graphicsDevice)
        {
            buttonSprite = exitButtonSprite;
            buttonScale = new Vector2(0.8f, 0.8f);
            buttonPosition = new Vector2((screenWidth / 2) - (buttonSprite.Width * buttonScale.X / 2), 680);
